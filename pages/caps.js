          ...
          {Object.keys(products).map((item) => {
            return <div key={products[item]._id} className="lg:w-1/5 md:w-1/2 p-4 w-full cursor-pointer shadow-lg m-5">
              <Link passHref={true} href={`/product/${products[item].slug}`} className="block relative  rounded overflow-hidden">
                <Image alt="ecommerce" className="block"  src={products[item].img} />
                <div className="mt-4">
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in cap) {
      if (!cap[item.title].color.includes(item.color) && item.availableQty > 0) {
        cap[item.title].color.push(item.color)
      }
      if (!cap[item.title].size.includes(item.size) && item.availableQty > 0) {
