    ...
    <section className="text-gray-600 body-font">
      <div className="container px-5 py-24 mx-auto">
        <div className="flex flex-wrap items-center justify-center -m-4">
          {Object.keys(products).length === 0 && <p className='md:text-2xl md:my-36 md:py-56  mx-5 text-center my-36 py-28'>Sorry all the mousepads are corrently out of stock. New stock comming soon. Stay Tuned.</p>}
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in mousepad) {
      if (!mousepad[item.title].color.includes(item.color) && item.availableQty > 0) {
        mousepad[item.title].color.push(item.color)
      }
      if (!mousepad[item.title].size.includes(item.size) && item.availableQty > 0) {
        mousepad[item.title].size.push(item.size)
      }
    }
    else {
      mousepad[item.title] = JSON.parse(JSON.stringify(item))
      if (item.availableQty > 0) {
        mousepad[item.title].color = [item.color]
        mousepad[item.title].size = [item.size]
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(mousepad)) }
  }
}
